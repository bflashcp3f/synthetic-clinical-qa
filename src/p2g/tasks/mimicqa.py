...
from p2g.templates.mimicqa import * 
...
class MIMICQAItem:
    ...
    def prompt_wrap(self, template_name: str, template: str, question_num: int = 1, paraphrase_num: int = 1):
        ...
        if template_name.startswith('question_generation'):
            if template_name in ['question_generation_plain', 'question_generation_explicit', 'question_generation_nonoverlap']:
                ...
                raise ValueError(f"Unrecognized template '{template_name}' for MIMICQA")
            ...
            assert "{{input_context}}" in template
            prompt = template.replace("{{input_context}}", self.context)
        elif template_name.startswith('qa_pair_generation'):
            assert "{{input_context}}" in template
            prompt = template.replace("{{input_context}}", self.context)
    ...
    def update_output(self, template_name: str, response: str):
        ...
        if template_name.startswith('question_generation'):
            ...
            qa_list = [item for item in response.strip().split('\n\n') if item.startswith('Q')]
            qas_generated = '\n\n'.join(qa_list)
            qas_generated = re.sub(r'Q\d+:', 'Q:', qas_generated)
            qas_generated = re.sub(r'A\d+:', 'A:', qas_generated)
            self.qas_generated = qas_generated
        else:
            raise ValueError(f"Unrecognized template '{template_name}' for MIMICQA")
...
class MIMICQADocItem:    
    ...
    def extract_questions(response):
        ...
        # Find all matches with multiline flag enabled
        questions = re.findall(pattern, response, re.M)
        assert len(questions) >= 1, f"Expected at least 1 match, got {len(questions)}"
...
class MIMICQA(Task):
    ...
    def load_data(self, data_path:str, start_index: int, end_index: int) -> list:
        ...
        if self.args.prior_template is None:
            ...
            if self.args.process_unit == 'paragraph':
                data = [MIMICQAItem(paragraph) for item in data['data'] for paragraph in item['paragraphs']]
            elif self.args.process_unit == 'qa_pair':
                data = [MIMICQAPairItem(qa['id'], qa['question'], qa['answers'], paragraph['context']) for item in data['data'] for paragraph in item['paragraphs'] for qa in paragraph['qas']]
            elif self.args.process_unit == 'document':
                data = [MIMICQADocItem(item) for item in data['data']]
            else:
                raise ValueError(f"Unrecognized process unit '{self.args.process_unit}' for MIMICQA")
            ...
            if self.args.prior_process_unit is None or self.args.prior_process_unit == self.args.process_unit:
                if self.args.process_unit == 'paragraph':
                    data = [MIMICQAItem(para_item) for para_item in data]
                elif self.args.process_unit == 'document':
                    data = [MIMICQADocItem(doc_item) for doc_item in data]
                else:
                    raise ValueError(f"Unrecognized process unit '{self.args.process_unit}' for MIMICQA")
            else:
                if self.args.prior_process_unit == 'paragraph':
                    data_prior = [MIMICQAItem(para_item) for para_item in data]
                elif self.args.prior_process_unit == 'document':
                    data_prior = [MIMICQADocItem(doc_item) for doc_item in data]
                else:
                    raise ValueError(f"Unrecognized prior process unit '{self.args.prior_process_unit}' for MIMICQA")
                ...
                if self.args.prior_process_unit == 'document' and self.args.process_unit == 'paragraph':
                    ...
                    data = [MIMICQAPairItem(qa['id'], qa['question'], qa['answers'], qa['context']) for qa in data]
                else:
                    raise ValueError(f"Unrecognized prior process unit '{self.args.prior_process_unit}' and process unit '{self.args.process_unit}' for MIMICQA")
    ...
    def get_icl_examples(self, item_idx, item) -> list:
        if self.args.icl_style == 'random':
            ...
            raise ValueError(f"Unrecognized icl style '{self.args.icl_style}' for MIMICQA")
    ...
    def get_output_data_path(self, template: str) -> str:
        ...
        if template.startswith('question_generation') or template.startswith('answer_generation') or template.startswith('qa_pair_generation'):
    ...
    def save_output(self) -> None:
        ...
        with open(output_data_path, 'w') as f:
            for item in self.data:
                # Dump the MIMICQAItem object as a json string
