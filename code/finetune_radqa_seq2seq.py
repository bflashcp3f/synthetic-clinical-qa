...
class DataTrainingArguments:
    ...
    cv_idx: int = field(
        default=None,
        metadata={"help": "An optional index for K-fold cross validation."},
    )
    num_fold: int = field(
        default=None,
        metadata={"help": "An optional number of folds for cross validation."},
    )
    replace_text_with_tags: bool = field(
        default=False, metadata={"help": "Replace text with tags, only for MIT clinical-t5"}
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.cv_idx:
            merged_train_val = concatenate_datasets([data_files['train'], data_files['validation']])
            dataset_chunk_list = [merged_train_val.shard(num_shards=data_args.num_fold, index=i) for i in range(data_args.num_fold)]
            val_cv = dataset_chunk_list[data_args.cv_idx]
            train_cv = concatenate_datasets(dataset_chunk_list[:data_args.cv_idx] + dataset_chunk_list[data_args.cv_idx + 1:])
            cv_dataset_dict = datasets.DatasetDict({"train": train_cv, "validation": val_cv, "test": data_files['test']})
            raw_datasets = datasets.DatasetDict(cv_dataset_dict)
        else:
    ...
    def preprocess_function(examples):
        ...
        inputs = replace_list_of_notes(inputs) if data_args.replace_text_with_tags else inputs
    ...
    def preprocess_validation_function(examples):
        ...
        print('LABEL OUT LEN =', len(labels_out))
        return model_inputs
    ...
    if training_args.do_predict:
        ...
        if data_args.cv_idx is not None:
            with open(training_args.output_dir + 'prediction.txt', 'w+') as f:
                for line in results.predictions:
                    f.write(f"{line}\n")
            with open(training_args.output_dir + 'label.txt', 'w+') as f:
                for line in results.label_ids:
                    f.write(f"{line}\n")
            with open(training_args.output_dir + 'scores.json', 'w+') as f:
                json.dump(results.metrics, f)
        else:
            with open(training_args.output_dir + 'original-prediction.txt', 'w+') as f:
                for line in results.predictions:
                    f.write(f"{line}\n")
            with open(training_args.output_dir + 'original-label.txt', 'w+') as f:
                for line in results.label_ids:
                    f.write(f"{line}\n")
            with open(training_args.output_dir + 'original-scores.json', 'w+') as f:
                json.dump(results.metrics, f)
